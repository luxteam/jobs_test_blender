...
def main(format):
	...
	if (format == 'JPEG'):
		format = 'JPG'
	# LOG
	name_scene_for_json = bpy.path.basename(bpy.context.blend_data.filepath) + "_" + format.lower() + "_" + "BL"
	...
	report['file_name'] = bpy.path.basename(bpy.context.blend_data.filepath) + "_01." + format.lower()
	report['scene_name'] = bpy.context.scene.name
	report['render_time'] = Render_time.total_seconds()
	report['render_color_path'] = r"{work_dir}" + "/Color/" + name_scene + "_01." + format.lower()
