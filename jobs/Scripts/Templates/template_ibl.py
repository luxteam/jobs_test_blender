...
def prerender(test_list):
	...
	if test_list[5]:
		...
		if type(test_list[6]) == tuple:
			set_value(scene.world.rpr.ibl, '{{}}_color'.format(test_list[5]), test_list[6])
		else:
			bpy.ops.image.open(filepath="//Maps//{{}}".format(test_list[6]), directory="{resource_path}//Maps//", files=[{{"name":"{{}}".format(test_list[6])}}], \
																															relative_path=True, show_multiview=False)
			set_value(scene.world.rpr.ibl, '{{}}_image'.format(test_list[5]), bpy.data.images['{{}}'.format(test_list[6])])
