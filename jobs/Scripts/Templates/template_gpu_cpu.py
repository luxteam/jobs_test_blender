...
def main(device_type, dual):
	...
	bpy.context.user_preferences.addons["rprblender"].preferences.settings.device_type_plus_cpu = dual
	...
	if (device_type == 'gpu' and dual == True):
		mode = 'dual'
	else:
		mode = device_type
	# output
	name_scene = bpy.path.basename(bpy.context.blend_data.filepath) + "_" + mode
	...
	# LOG
	name_scene_for_json = bpy.path.basename(bpy.context.blend_data.filepath) + "_" + mode + "BL"
